        ...
        if use_amp and amp_dtype is not None:
            with torch.autocast(device_type=device_type, dtype=amp_dtype):
                if model_class == 'BSMS_MeshGraphNet':
                    ...
                    spectral_features = batch.spectral_features if hasattr(batch, 'spectral_features') else None
                    pred = model(batch.x, batch.edge_attr, batch.edge_index, batch.batch, spectral_features)

                elif model_class == 'GraphDistanceTransolver':
                    # GraphDistanceTransolver uses batch tensor and requires graph_distances
                    # Check for graph_distances_list first (from GraphDistanceDataLoader)
                    if hasattr(batch, 'graph_distances_list'):
                        graph_distances = batch.graph_distances_list
                    elif hasattr(batch, 'graph_distances'):
                        graph_distances = batch.graph_distances
                    else:
                        graph_distances = None
                    pred = model(batch.x, batch.edge_attr, batch.edge_index, batch.batch, graph_distances)
...
def create_model(model_config: dict, 
    ...
    if model_name == 'MLP' or model_name == 'mlpnet':
        ...
        from models.graphSpectralTransolver import GraphSpectralTransolver
        model = GraphSpectralTransolver(
            input_node_dim=input_node_dim,
            output_node_dim=output_node_dim,
            space_dim=pos_dim,  # 2 for 2D, 3 for 3D
            n_layers=model_config.get('n_layers', 6),
            ...
            mlp_ratio=model_config.get('mlp_ratio', 4),
            slice_num=model_config.get('slice_num', 32),
            spectral_dim=model_config.get('spectral_dim', 8),
            spectral_weight=model_config.get('spectral_weight', 0.5),
            use_checkpoint=model_config.get('use_checkpoint', True),
            fourier_features=model_config.get('fourier_features', False),
            fourier_dim=model_config.get('fourier_dim', 0),
            laplacian_norm=model_config.get('laplacian_norm', 'sym'),
        )

    elif model_name == 'graphdistance_transolver':
        from models.graphDistanceTransolver import GraphDistanceTransolver
        model = GraphDistanceTransolver(
            input_node_dim=input_node_dim,
            output_node_dim=output_node_dim,
            space_dim=pos_dim,  # 2 for 2D, 3 for 3D
            n_layers=model_config.get('n_layers', 6),
            ...
            mlp_ratio=model_config.get('mlp_ratio', 4),
            slice_num=model_config.get('slice_num', 32),
            distance_bias_type=model_config.get('distance_bias_type', 'exp'),
            learnable_bias=model_config.get('learnable_bias', True),
            use_checkpoint=model_config.get('use_checkpoint', True),
            fourier_features=model_config.get('fourier_features', False),
            fourier_dim=model_config.get('fourier_dim', 0),
        )

    elif model_name == 'mgntransolver':
        from models.mgntransolver import MGNTransolver
        model = MGNTransolver(
            input_node_dim=input_node_dim,
            input_edge_dim=input_edge_dim,
            output_node_dim=output_node_dim,
            ...
            hidden_dim_edge_encoder=model_config.get('hidden_dim', 128),
            aggregation=model_config.get('aggregation', 'sum'),
